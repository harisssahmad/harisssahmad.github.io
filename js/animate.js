...
const animateInitSMSimple = document.querySelectorAll(
    ".animate-init-sm-simple"
);
...
const animateInitLGSimple = document.querySelectorAll(
    ".animate-init-lg-simple"
);
...
const checkAndAddAnimation = (element, addClass) => {
    if (isScrolledIntoView(element) && !element.classList.contains(addClass)) {
        ...
        element.classList.contains(addClass)
    ) {
...
if (pageWidth <= 991) {
    ...
    animateInitSMLeft.forEach((element) =>
        checkAndAddAnimation(element, "animate__fadeInLeft")
    );

    // initial right sm elements
    animateInitSMRight.forEach((element) =>
        checkAndAddAnimation(element, "animate__fadeInRight")
    );

    // initial simple sm elements
    animateInitSMSimple.forEach((element) =>
        checkAndAddAnimation(element, "animate__fadeIn")
    );
} else {
    // initial left sm elements
    animateInitLGLeft.forEach((element) =>
        checkAndAddAnimation(element, "animate__fadeInLeft")
    );

    // initial right sm elements
    animateInitLGRight.forEach((element) =>
        checkAndAddAnimation(element, "animate__fadeInRight")
    );

    // initial simple lg elements
    animateInitLGSimple.forEach((element) =>
        checkAndAddAnimation(element, "animate__fadeIn")
    );
}
...
const handleScroll = () => {
    ...
    if (currentScrollPos > prevScrollPos) {
        ...
        // down elements
        animateElementsDown.forEach((element) =>
            checkAndAddAnimation(element, "animate__fadeInDown")
        );

        // up elements
        animateElementsUp.forEach((element) =>
            checkAndAddAnimation(element, "animate__fadeInUp")
        );

        // simple elements
        animateElementsSimple.forEach((element) =>
            checkAndAddAnimation(element, "animate__fadeIn")
        );
        ...
        if (pageWidth > 991) {
            // left elements
            animateElementsLGLeft.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInLeft")
            );

            // right elements
            animateElementsLGRight.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInRight")
            );

            // up elements
            animateElementsLGUp.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInUp")
            );

            // down elements
            animateElementsLGDown.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInDown")
            );

            // simple elements
            animateElementsLGSimple.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeIn")
            );
        } else {
            // SM size
            ...
            // left elements
            animateElementsSMLeft.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInLeft")
            );

            // right elements
            animateElementsSMRight.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInRight")
            );

            // up elements
            animateElementsSMUp.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInUp")
            );

            // down elements
            animateElementsSMDown.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeInDown")
            );

            // simple elements
            animateElementsSMSimple.forEach((element) =>
                checkAndAddAnimation(element, "animate__fadeIn")
            );
        }
    }
